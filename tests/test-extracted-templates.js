...
async function main() {
    ...
    for (const r of results) {
        console.log(
            `| ${r.source.padEnd(6)} | ${String(r.templateCount).padStart(9)} | ${String(r.truePositives).padStart(2)} | ${String(r.falsePositives).padStart(2)} | ${String(r.falseNegatives).padStart(2)} | ${r.precision.toFixed(1).padStart(9)}% | ${r.recall.toFixed(1).padStart(5)}% | ${r.f1.toFixed(1).padStart(7)}% |`
        );
    }
