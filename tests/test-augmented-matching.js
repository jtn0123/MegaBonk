...
async function main() {
    ...
    for (const r of [originalResult, augmentedResult, combinedResult]) {
        console.log(
            `| ${r.name.padEnd(28)} | ${(r.precision * 100).toFixed(1).padStart(8)}% | ${(r.recall * 100).toFixed(1).padStart(6)}% | ${(r.f1 * 100).toFixed(1).padStart(7)}% |`
        );
    }
    ...
    console.log(
        `Combined vs Original: ${combinedImprovement >= 0 ? '+' : ''}${combinedImprovement.toFixed(1)}% F1 change`
    );
}
