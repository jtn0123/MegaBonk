...
const CHANGES_TO_TRY: Array<{ param: keyof ExperimentConfig; value: number; expectedGain: string }> = [
    { param: 'centerMargin', value: 0.2, expectedGain: '+12.3%' },
    { param: 'agreementThreshold', value: 0.2, expectedGain: '+12.1%' },
...
async function main() {
    ...
    console.log(
        `| ${'Change'.padEnd(30)} | ${'Expected'.padEnd(10)} | ${'Actual F1'.padEnd(12)} | ${'Keep?'.padEnd(6)} |`
    );
    console.log('-'.repeat(70));
    ...
    for (const change of CHANGES_TO_TRY) {
        ...
        console.log(
            `| ${changeDesc.padEnd(30)} | ${change.expectedGain.padEnd(10)} | ${deltaStr.padEnd(12)} | ${keepStr.padEnd(6)} |`
        );
