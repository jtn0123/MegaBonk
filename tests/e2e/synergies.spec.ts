...
test.describe('Synergies - Build Planner', () => {
    ...
    test('should detect character-weapon synergy (Sir Oofie + Sword)', async ({ page }) => {
        ...
        const characterOptions = await page.locator('#build-character option').allTextContents();
        const oofieIndex = characterOptions.findIndex(
            opt => opt.toLowerCase().includes('oofie') || opt.toLowerCase().includes('sir oofie')
        );
...
test.describe('Synergies - Visual Indicators', () => {
    ...
    test('synergy tags have consistent styling across tabs', async ({ page }) => {
        ...
        if (itemTagCount > 0) {
            // Get computed style
            const bgColor = await itemSynergyTags.first().evaluate(el => getComputedStyle(el).backgroundColor);
        ...
        if (charTagCount > 0) {
            const bgColor = await charSynergyTags.first().evaluate(el => getComputedStyle(el).backgroundColor);
            expect(bgColor).toBeDefined();
        }
    });
    ...
    test('anti-synergy tags have warning/distinct styling', async ({ page }) => {
        ...
        if (tagCount > 0) {
            ...
            const antiColor = await antiSynergyTags.first().evaluate(el => getComputedStyle(el).backgroundColor);

            const synergyTags = page.locator('#modalBody .synergy-tag');
            const synergyTagCount = await synergyTags.count();

            if (synergyTagCount > 0) {
                const synergyColor = await synergyTags.first().evaluate(el => getComputedStyle(el).backgroundColor);
