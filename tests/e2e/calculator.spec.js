...
test.describe('Calculator', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('#calc-item', { timeout: 5000 });
  });
  ...
  test('should display item dropdown with options', async ({ page }) => {
    const options = page.locator('#calc-item option');
  ...
  test('should display breakpoint presets', async ({ page }) => {
    const presets = page.locator('.preset-btn');
    const count = await presets.count();

    // Should have breakpoint preset buttons
  ...
  test('should show base value input field', async ({ page }) => {
    const baseInput = page.locator('#calc-base');
    await expect(baseInput).toBeVisible();
  });

  test('should calculate stacks needed when item selected', async ({ page }) => {
    // Select an item
    await page.selectOption('#calc-item', { index: 1 });
  ...
  test('should use preset breakpoint values', async ({ page }) => {
    // Click first preset button
    await page.click('.preset-btn >> nth=0');

    // Target input should have a value
    const targetValue = await page.locator('#calc-target').inputValue();
    expect(targetValue).not.toBe('');
    expect(Number(targetValue)).toBeGreaterThan(0);
  });
  ...
  test('should clear result when item changes', async ({ page }) => {
    ...
    await page.selectOption('#calc-item', { index: 2 });
