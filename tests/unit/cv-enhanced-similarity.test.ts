...
import {
    ...
    calculateSSD,
    calculateSSIM,
} from '../../src/modules/cv-enhanced/similarity.ts';
...
describe('cv-enhanced/similarity', () => {
    describe('calculateSimilarity', () => {
        it('dispatches to NCC', () => {
            expect(calculateSimilarity(mkGradient(4, 4, 50, 200), mkGradient(4, 4, 50, 200), 'ncc')).toBeGreaterThan(
                0.9
            );
        });
        ...
        it('dispatches to SSIM', () => {
            expect(calculateSimilarity(mkGradient(4, 4, 50, 200), mkGradient(4, 4, 50, 200), 'ssim')).toBeGreaterThan(
                0.8
            );
        });
    });
    ...
    describe('calculateSSD', () => {
        it('returns 1 for identical', () => {
            expect(calculateSSD(mkImg(4, 4, 128), mkImg(4, 4, 128))).toBeCloseTo(1, 5);
        });
        it('low for opposite', () => {
            expect(calculateSSD(mkImg(4, 4, 0), mkImg(4, 4, 255))).toBeLessThan(0.01);
        });
    ...
    describe('calculateSSIM', () => {
        it('high for identical gradients', () => {
            expect(calculateSSIM(mkGradient(8, 8, 50, 200), mkGradient(8, 8, 50, 200))).toBeGreaterThan(0.9);
        });
        it('lower for reversed', () => {
            expect(calculateSSIM(mkGradient(8, 8, 0, 200), mkGradient(8, 8, 200, 0))).toBeLessThan(0.9);
        });
        it('no NaN for uniform', () => {
            expect(calculateSSIM(mkImg(4, 4, 128), mkImg(4, 4, 128))).not.toBeNaN();
        });
    });
});
