...
describe('cv/detection-processing extended', () => {
    ...
    describe('boostConfidenceWithContext', () => {
        ...
        it('clamps to 0.99', () => {
            const r = boostConfidenceWithContext([
                mkDet({ entity: { name: 'T', rarity: 'common' }, confidence: 0.99 }),
            ]);
            expect(r[0]!.confidence).toBeLessThanOrEqual(0.99);
        });
        it('empty input', () => {
            expect(boostConfidenceWithContext([])).toEqual([]);
        });
    });
    ...
    describe('filterByConfidence', () => {
        it('filters low', () => {
            expect(
                filterByConfidence(
                    [
                        mkDet({ entity: { name: 'A' }, confidence: 0.9 }),
                        mkDet({ entity: { name: 'B' }, confidence: 0.3 }),
                    ],
                    0.5
                )
            ).toHaveLength(1);
    ...
    describe('aggregateDetections', () => {
        it('merges arrays', () => {
            expect(
                aggregateDetections([mkDet({ entity: { name: 'A' } })], [mkDet({ entity: { name: 'B' } })])
            ).toHaveLength(2);
        });
        it('handles empty', () => {
            expect(aggregateDetections([], [])).toHaveLength(0);
        });
    });

    describe('getCachedResults/cacheResults', () => {
        it('null for unknown', () => {
            expect(getCachedResults('xxx_' + Date.now())).toBeNull();
        });
        it('round-trips', () => {
            const h = 'h_' + Date.now();
