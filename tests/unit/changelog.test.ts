...
describe('Changelog Module', () => {
    ...
    describe('renderChangesSections()', () => {
        ...
        it('should parse entity links in raw notes', () => {
            const rawNotes = 'Fixed [[item:bonk_stick|Bonk Stick]] damage calculation.';
            const result = renderChangesSections(null, rawNotes);
        ...
        it('should escape HTML while preserving entity links in raw notes', () => {
            const rawNotes = 'Fixed [[item:bonk_stick|Bonk Stick]] <script>xss</script>';
            const result = renderChangesSections(null, rawNotes);

            // Should have entity link
            expect(result).toContain('entity-link');
            expect(result).toContain('>Bonk Stick</a>');
            // Should escape XSS attempt
            expect(result).not.toContain('<script>');
            expect(result).toContain('&lt;script&gt;');
        });
