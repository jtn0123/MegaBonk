...
async function main() {
    ...
    for (const config of configs) {
        ...
        console.log(
            `| ${config.name.padEnd(26)} | ${String(result.totalTP).padStart(2)} | ${String(result.totalFP).padStart(2)} | ${String(result.totalFN).padStart(3)} | ${(result.precision * 100).toFixed(1).padStart(8)}% | ${(result.recall * 100).toFixed(1).padStart(5)}% | ${(result.f1 * 100).toFixed(1).padStart(7)}% |`
        );
    }
